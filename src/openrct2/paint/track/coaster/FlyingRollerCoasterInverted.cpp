...
{
    height += 5; // compensate for TrackDrawerEntry::heightOffset = -5
...
static void InvertedFlyingRCTrackDiagBlockBrakes(
    ...
    const TrackElement& trackElement, SupportType supportType)
{
    // DiagBlockBrakes uses the same rendering as DiagBrakes for this coaster.
    GetTrackPaintFunctionInvertedRC(TrackElemType::diagBrakes)(
...
TrackPaintFunction GetTrackPaintFunctionFlyingRCInverted(TrackElemType trackType)
{
    switch (trackType)
    {
        ...
        default:
            return GetTrackPaintFunctionInvertedRC(trackType);
    }
}
